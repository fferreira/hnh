...
where $x$ is the name of the functions and the $p$'s are the different patterns. One limitation of HNH is that we use patterns other variable patterns, the interpreter supports functions with only one parameter. This limitation may be removed for the final compiler.
